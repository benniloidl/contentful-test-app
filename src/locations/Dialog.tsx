...
export const API_URL = "https://api.cvy8qx-unicgmbha1-d1-public.model-t.cc.commerce.ondemand.com";
...
export default function Dialog() {
	...
	useEffect(() => {
		let url = API_URL + "/occ/v2/basesites?fields=FULL";
		fetch(url)
	...
	useEffect(() => {
		...
		let url = API_URL + "/occ/v2/" + selectedBaseSite.uid + "/products/search?query=" + encodeURIComponent(query) + "&fields=FULL&currentPage=" + page;
		fetch(url)
			.then(response => response.json())
			.then(data => {
		...
		fetch(API_URL + "/occ/v2/" + selectedBaseSite.uid + "/catalogs")
			.then(response => response.json())
			.then(data => {
			...
			{ categories.map((category: any, index: number) => {
				return <Flex gap={ "spacingM" } key={ index }>
					<Button variant={ "primary" }
							onClick={ () => sdk.close({
								type: "category",
								data: API_URL + "/occ/v2/" + (selectedBaseSite?.uid ?? "") + "/categories/" + category.id + "/products?pageSize=3"
							}) }>
						{ category.name }
					</Button>
										...
										<TableCell style={ { paddingBlock: 0 } }>
											<Flex alignItems={ "center" } justifyContent={ "center" }>
												{ product.firstVariantImage ?
													<img src={ API_URL + product.firstVariantImage } alt={ product.name }
										...
										<TableCell style={ { height: "100%" } }>
											<Flex flexDirection={ "row" } alignItems={ "center" } justifyContent={ "space-between" } style={ { height: "100%" } }>
												...
												<Button variant={ "primary" }
														onClick={ () => sdk.close({
															type: "product",
															data: API_URL + "/occ/v2/" + (selectedBaseSite?.uid ?? "") + "/products/" + product.code
														}) }>
													Select
												</Button>
											</Flex>
